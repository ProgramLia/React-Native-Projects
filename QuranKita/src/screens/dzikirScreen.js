...
// BUILD-IN-FUNCTION...
export default function DzikirScreen({ navigation, route }) {
     // DATA...
     const { data , name , isDoa} = route.params;
               ...
               <FlatList contentContainerStyle={{ padding: 10, gap: 10 }} data={data} renderItem={({ item, index }) => {
                    return (
                         <View style={[style.contentContainer, { backgroundColor: Scheme === "dark" ? "#333" : "#fff", color: Scheme === "dark" ? "#f0f0f0" : "#333" }]}>
                              {isDoa === false ? <Text style={[style.textArab, { color: Scheme === "dark" ? "#f0f0f0" : "#333", fontFamily:translate ? "Poppins-Medium" : "arial" , fontSize: translate ? 20 : 32 }]}>{translate === false ? toArabic(item.ulang) : item.ulang}</Text> : <Text style={[style.textArab, { color: Scheme === "dark" ? "#f0f0f0" : "#333", fontFamily:"Poppins-Medium", fontSize: 20 }]}>{item.judul}</Text>}
                              <Text style={[style.textArab, { color: Scheme === "dark" ? "#f0f0f0" : "#333" }]}>{item.arab}</Text>
                              {translate ? <Text style={[style.textArab, { fontFamily: "Poppins-Regular", fontSize: 14, color: Scheme === "dark" ? "#f0f0f0" : "#333" }]}>{item.indo}</Text> : null}
                         </View>
                    )
               }} />
          </View>
